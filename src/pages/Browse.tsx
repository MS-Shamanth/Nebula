...
const Browse = () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(article => article.title.toLowerCase().includes(query) || article.excerpt.toLowerCase().includes(query) || article.tags.some(tag => tag.toLowerCase().includes(query)) || article.author.toLowerCase().includes(query));
    }
  ...
  const handleSentimentToggle = (sentiment: SentimentType) => {
    setSelectedSentiments(prev => prev.includes(sentiment) ? prev.filter(s => s !== sentiment) : [...prev, sentiment]);
  };
  const handleKarma = (id: string) => {
    setArticles(prev => prev.map(article => article.id === id ? {
      ...article,
      karma: article.karma + 1
    } : article));
  };
  return <div className="min-h-screen bg-gradient-hero">
      <main className="container mx-auto px-4 py-12 space-y-8">
        ...
        <section className="text-center space-y-4">
          <h1 className="text-4xl md:text-5xl font-bold text-violet-600">
            Browse Knowledge
          </h1>
        ...
        <div className="max-w-3xl mx-auto">
          <SearchBar onSearch={setSearchQuery} placeholder="Search by title, author, tags, or content..." />
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="lg:w-64 space-y-6">
            <Button variant="outline" onClick={() => setShowFilters(!showFilters)} className="w-full lg:hidden">
              <SlidersHorizontal className="w-4 h-4 mr-2" />
              {showFilters ? "Hide" : "Show"} Filters
            </Button>

            <div className={`space-y-6 ${showFilters ? "block" : "hidden lg:block"}`}>
              <SentimentFilter selectedSentiments={selectedSentiments} onToggle={handleSentimentToggle} />
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  value: "views",
                  label: "Most Viewed"
                }].map(option => <Button key={option.value} variant={sortBy === option.value ? "default" : "outline"} size="sm" onClick={() => setSortBy(option.value as typeof sortBy)} className="justify-start">
                      {option.label}
                    </Button>)}
                </div>
              </div>
          ...
          <div className="flex-1">
            <div className="mb-4 text-sm text-muted-foreground bg-transparent rounded-md">
              Showing {filteredArticles.length} {filteredArticles.length === 1 ? "article" : "articles"}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredArticles.map(article => <ArticleCard key={article.id} article={article} onKarma={handleKarma} />)}
            </div>
